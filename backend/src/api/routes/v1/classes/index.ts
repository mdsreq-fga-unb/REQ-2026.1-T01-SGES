    ...
    for (const id of instructorIds) {
      ...
      if (!user) {
        throw new NotFoundError(
          `Instrutor com ID "${id}" não encontrado ou está inativo.`,
        );
      }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      // Adiciona studentsCount para cada turma (vagas ocupadas)
      const classesWithCount = await Promise.all(
        classes.map(async (c) => {
          const studentsCount =
            await enrollmentRepo.countActiveEnrollmentsByClass(c.id);
...
router.post("/", authMiddleware([UserRole.ADMIN]), async (req, res, next) => {
  try {
    ...
    const duplicate = await classRepo.findByDetails(
      nomeCurso,
      diaSemana,
      horario,
    );
  ...
  async (req, res, next) => {
    try {
      ...
      const mergedBody = {
        ...
        livrosEstudados:
          req.body.livrosEstudados ?? existingClass.livrosEstudados,
        vagasLimite: req.body.vagasLimite ?? existingClass.vagasLimite,
        semestre: req.body.semestre ?? existingClass.semestre ?? "2026.1",
...
router.get(
  "/:classId/students",
  authMiddleware([UserRole.ADMIN, UserRole.TEACHER]),
  listClassStudentsRoute,
);
router.post(
  "/:classId/students",
  authMiddleware([UserRole.ADMIN, UserRole.TEACHER]),
  enrollClassStudentRoute,
);
router.delete(
  "/:classId/students/:studentId",
  authMiddleware([UserRole.ADMIN, UserRole.TEACHER]),
  unenrollClassStudentRoute,
);
router.post(
  "/:classId/attendances",
  authMiddleware([UserRole.ADMIN, UserRole.TEACHER]),
  registerClassAttendanceRoute,
);
router.put(
  "/:classId/attendances",
  authMiddleware([UserRole.ADMIN]),
  alterClassAttendanceRoute,
);
